...
import { NanoGptProviderV2 } from '@/lib/ai/nano-gpt-v2'
import { AIGenerationService } from '@/lib/ai/ai-generation-service'

interface GenerationLoungeV2Props {
  selectedCostume: CostumePresetV2
  userEmail?: string
...
const SUPPORTED_MODELS = ['seedream-v4', 'google:4@1', 'background-remover'] as const

const getDefaultNanoGptModel = () => {
	const envModel = import.meta.env.VITE_DEFAULT_MODEL
	if (envModel && SUPPORTED_MODELS.includes(envModel as any)) {
		return envModel as any
	}
	return 'seedream-v4'
}
...
export const GenerationLoungeV2 = ({
	selectedCostume,
	userEmail,
	selfieBase64,
	uploadedSelfie,
	onComplete,
}: GenerationLoungeV2Props) => {
	...
	const stages: GenerationProgress[] = useMemo(
		() => [
			...
			},
			{
				stage: 'Upload',
				progress: 35,
				message: 'Sending your selfie to the AI atelier...',
	...
	useEffect(() => {
		logEvent('generation_lounge_v2_entered', {
			costumeId: selectedCostume.id,
			costumeName: selectedCostume.name,
			userEmail: userEmail || 'guest',
			hasAIGenerationSettings: !!selectedCostume.aiGeneration,
		})
	...
	useEffect(() => {
		...
		const executeGeneration = async () => {
			...
			try {
				// Build AI generation request using costume-specific settings
				...
				logEvent('generation_v2_request_built', {
					...debugInfo,
					attempt,
				})

				logEvent('generation_v2_request_dispatched', {
					costumeId: selectedCostume.id,
					model: aiRequest.model,
					references: aiRequest.references.length,
					attempt,
					hasSeed: !!aiRequest.options?.seed,
					hasNegativePrompt: !!aiRequest.negativePrompt,
				})
				...
				if (!primaryImage) {
					throw new Error('AI provider did not return any images')
				}
				...
				if (!imageUrl) {
					throw new Error('AI response missing usable image data')
				}
				...
				logEvent('generation_v2_completed', {
					costumeId: selectedCostume.id,
					costumeName: selectedCostume.name,
					model: aiRequest.model,
					referenceCount: aiRequest.references.length,
					attempt,
					seed: aiRequest.options?.seed,
				...
				logError('generation_v2_failed', error, {
					costumeId: selectedCostume.id,
					model: selectedCostume.aiGeneration.model,
					attempt,
	...
	useEffect(() => {
		...
		logEvent('generation_v2_stage_updated', {
			stage: stage.stage,
			progress,
			costumeId: selectedCostume.id,
			status,
	...
	const handleRetry = useCallback(() => {
		logEvent('generation_v2_retry_requested', {
			costumeId: selectedCostume.id,
			attempt: attempt + 1,
		})
		setProgress(5)
	...
	return (
				...
				<div className="space-y-1">
					...
					<p className="text-muted-foreground">
						{status === 'config-missing'
							? 'Add your AI credentials to continue your transformation'
          ...
          {status === 'config-missing' && (
            <div className="flex items-center gap-3 rounded-lg border border-amber-300 bg-amber-50 p-4 text-left">
              ...
              <div className="space-y-1 text-sm text-amber-800">
                <p className="font-medium">AI configuration required</p>
                <p>
      ...
      {/* Costume Info with AI Settings Display */}
          ...
          <div className="flex-1">
            ...
            <div className="mt-2">
              <Badge variant="outline" className="text-xs">
                🤖 {selectedCostume.aiGeneration.model} • Seed: {selectedCostume.aiGeneration.seed}
              </Badge>
            </div>
          </div>
        </div>
