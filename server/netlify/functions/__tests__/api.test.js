...
const handlerImplementation = async (request) => new Response(`handled:${new URL(request.url).pathname}`, { status: 200 });
...
const setHandlerOverride = () => {
    ...
    globalThis[GLOBAL_HANDLER_KEY] = handlerMock;
};
const clearHandlerOverride = () => {
    delete globalThis[GLOBAL_HANDLER_KEY];
};
const getCalledRequest = () => {
...
for (const { label, loader } of moduleLoaders) {
    describe(`${label} handler`, () => {
        ...
        it('normalizes nested Netlify function paths while preserving query', async () => {
            const request = new Request('https://example.com/.netlify/functions/api/costumes/featured?limit=4');
