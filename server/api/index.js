import { getAllCostumes, getCostumeById, getFeaturedCostumes, } from './neon-client.js';
const ALLOW_ORIGIN = process.env.API_ALLOW_ORIGIN ?? '*';
const LOG_SINK = process.env.LOG_SINK ?? 'stdout';
const parseAllowedOrigins = () => ALLOW_ORIGIN.split(',')
...
const jsonResponse = (body, status, origin) => withCors(new Response(JSON.stringify(body, null, 2), {
    status,
    headers: {
        'Content-Type': 'application/json',
    },
}), origin);
const emptyResponse = (status, origin) => withCors(new Response(null, { status }), origin);
const notFound = (origin) => jsonResponse({ error: 'Not found' }, 404, origin);
const handleApiRequest = async (url, origin) => {
...
const isLogLevel = (level) => level === 'info' || level === 'error';
const emitLog = (entry) => {
    const payload = {
        ...entry,
...
const handleLogIngest = async (request, origin) => {
    let payload;
    try {
...
async function handler(request) {
    ...
    const url = new URL(request.url, process.env.VERCEL_URL
        ? `https://${process.env.VERCEL_URL}`
        : 'http://localhost:4000');
    const requestOrigin = typeof request.headers?.get === 'function'
...
if (typeof globalThis.Bun !== 'undefined' && !process.env.VERCEL) {
    console.log(`🌀  Running local Bun server on http://localhost:${API_PORT}`);
    const server = globalThis.Bun.serve({ port: API_PORT, fetch: handler });
