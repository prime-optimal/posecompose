...
Before making first commits on any new feature branch, ensure these are excluded:
- **`.factory/`** - Custom droid configurations and internal project settings
- **`AGENTS.md`** - This guide (internal documentation only)
- **Environmental files**: `.env`, `.envrc`, `.env.*` - contains secrets and API keys
- **Platform files**: `.vercel`, database files, cache/temp directories  
- **Backups**: `*.bak`, `*.backup` - development artifacts

Use `git rm --cached <file>` to remove tracking without deleting local files if something was accidentally committed.

### Feature Branch Workflow
Always create feature branches for sub-phase work: `feature/subphase-X-name`
This keeps main clean and enables CI/CD integration without conflicts.
...
2. **Implementation**
   - Coordinate responsibilities across droids:
     ...
     - `ai-pipeline-droid` – provider/model abstraction (`seedream-v4`, Gemini, OpenAI-compatible).
       - Maintain provider docs under `docs/nanogpt/` alongside any model-specific testing notes (e.g., 30-second integration timeouts).
       - Keep live NanoGPT tests opt-in by requiring `RUN_NANO_GPT_INTEGRATION=true` to manage quota usage.
       - Prefer direct asset URLs for references when models allow; use base64 with explicit MIME types for single-image models such as `background-remover`.
